...
export function MerchantCategoriesPage({ title, vertical, listHref }: Props) {
  ...
  useEffect(() => {
    if (vertical === 'restaurant' || !token) return;
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Taom kategoriyalari barcha restoranlar uchun umumiy. Restoranlar faqat mahsulot
              yaratishda tanlaydi.
            </p>
          </div>
          <Link href={listHref}>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold">{title}</h1>
          <p className="mt-1 text-sm text-zinc-500">
            Do&apos;kon mahsulot kategoriyalari — har bir do&apos;kon o&apos;z ro&apos;yxatini yuritadi.
          </p>
        </div>
        <Link href={listHref}>
          <Button type="button" variant="secondary">
      ...
      {!merchantId ? (
        <EmptyState title="Tanlang" description="Kategoriyalarni ko'rish uchun do'konni tanlang." />
      ) : selected ? (
