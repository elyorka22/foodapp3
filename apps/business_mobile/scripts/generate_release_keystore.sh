...
ALIAS="${ANDROID_KEY_ALIAS:-foodapp-business-release}"
...
openssl req -new -x509 -key "$KEY_PATH" -out "$CRT_PATH" -days 3650 \
  -subj "/CN=FoodApp Business/O=FoodApp/C=UZ"
...
echo "SHA256=${SHA256}"
echo ""
echo "Add SHA-1 and SHA-256 to Firebase Console → com.foodapp.business_mobile"
